...
- [ ] 0.5.2 Configurer les règles sur GitHub (Settings → Branches → Branch protection rules) ← **MANUEL**
...
- [x] 3.5.1 Créer `scripts/demo-rest.sh` (défaut testnet si env var absente) :
  ...
  #!/usr/bin/env bash
  set -euo pipefail
  TENDERMINT_RPC_URL="${TENDERMINT_RPC_URL:-https://cosmoshub.tendermintrpc.lava.build:443}"
  export TENDERMINT_RPC_URL
...
- [x] 3.5.2 Créer `scripts/demo-ws.sh` (défaut testnet si env var absente) :
  ...
  #!/usr/bin/env bash
  set -euo pipefail
  TENDERMINT_WS_URL="${TENDERMINT_WS_URL:-wss://cosmoshub.tendermintrpc.lava.build:443/websocket}"
  export TENDERMINT_WS_URL
...
- [x] 3.5.3 Créer `scripts/demo-grpc.sh` (défaut testnet si env var absente) :
  ...
  #!/usr/bin/env bash
  set -euo pipefail
  TENDERMINT_GRPC_URL="${TENDERMINT_GRPC_URL:-cosmoshub.grpc.lava.build}"
  export TENDERMINT_GRPC_URL
  dotnet run --project samples/CometBFT.Client.Demo.Grpc \
    --configuration Release "$@"
  ```

- [x] 3.5.4 Créer `scripts/docker/demo-rest.sh` — forward `TENDERMINT_RPC_URL` :
  ```bash
  #!/usr/bin/env bash
  set -euo pipefail
  docker run --rm \
    -v "$(pwd):/workspace" \
    -w /workspace \
    -e TENDERMINT_RPC_URL \
    mcr.microsoft.com/dotnet/sdk:10.0 \
    ./scripts/demo-rest.sh "$@"
  ```

- [x] 3.5.5 Créer `scripts/docker/demo-ws.sh` — forward `TENDERMINT_WS_URL` :
  ```bash
  #!/usr/bin/env bash
  set -euo pipefail
  docker run --rm \
...
- [x] 3.5.6 Créer `scripts/docker/demo-grpc.sh` — forward `TENDERMINT_GRPC_URL` :
  ```bash
  #!/usr/bin/env bash
  set -euo pipefail
  docker run --rm \
...
- [x] 4.2.1 `ITendermintRestClient.cs`
- [x] 4.2.2 `IHealthService`, `IStatusService`, `IBlockService`, `ITxService`, `IValidatorService`, `IAbciService`
- [x] 4.2.3 `ITendermintWebSocketClient.cs`
- [x] 4.2.4 `ITendermintGrpcClient.cs`
...
- [x] 4.2.6 Aligner `ITendermintRestClient`, `ITendermintWebSocketClient` et `ITendermintGrpcClient` sur les mêmes objets `Core.Domain` pour tout concept métier partagé (block, header, tx result, validator set, broadcast result, etc.)
...
- [x] 4.3.1 `TendermintRestOptions`, `TendermintWebSocketOptions`, `TendermintGrpcOptions`
- [x] 4.3.2 `TendermintClientException`, `TendermintRestException`, `TendermintWebSocketException`, `TendermintGrpcException`
...
- [x] 5.2.1 `TendermintWebSocketClient` avec `Websocket.Client 5.0.0`
...
- [x] 5.2.6 Étendre `ITendermintWebSocketClient` et `TendermintWebSocketClient` pour couvrir tout appel ou abonnement public manquant révélé par cet audit
...
- [x] 5.3.3 `TendermintGrpcClient` : `PingAsync`, `BroadcastTxAsync`
...
- [x] 5.3.6 Aligner le proto vendored local sur le proto amont exact de `v0.38.9` (`proto/tendermint/rpc/grpc/types.proto`), y compris `ResponseBroadcastTx.tx_result`
- [x] 5.3.7 Étendre `ITendermintGrpcClient` si nécessaire pour refléter exactement la surface publique gRPC auditée, avec `CancellationToken` sur chaque méthode
- [x] 5.3.8 Étendre `TendermintGrpcClient` et ses mappings de domaine pour représenter complètement `ResponseBroadcastTx` et toute autre réponse gRPC publique auditée
...
- [x] 5.4.4 `TendermintJsonContext : JsonSerializerContext` avec `[JsonSerializable]` pour tous les types REST (top-level + enveloppes génériques)
- [x] 5.4.5 Tous les appels `JsonSerializer.DeserializeAsync<T>` utilisent `TendermintJsonContext.Default.Options`
...
- [x] 5.4.9 Zéro appel `JsonSerializer` avec l'overload par défaut (réflexion) — POST uses `TendermintJsonContext.Default.JsonRpcBroadcastRequest`
...
- [x] 6.1 `AddTendermintRest(this IServiceCollection, Action<TendermintRestOptions>)`
- [x] 6.2 `AddTendermintWebSocket(this IServiceCollection, Action<TendermintWebSocketOptions>)`
- [x] 6.3 `AddTendermintGrpc(this IServiceCollection, Action<TendermintGrpcOptions>)`
...
- [x] 7.2.3 Tests erreurs (JSON-RPC error → TendermintRestException)
...
- [x] 7.3.2 Tests souscription/désouscription sans connexion → TendermintWebSocketException
...
- [x] 7.4.2 `BroadcastTxAsync` → résultat / null / RpcException → TendermintGrpcException
...
- [x] 7.5.1 Vérifier enregistrements DI (AddTendermintRest/WebSocket/Grpc)
...
- [x] 7.6.2 Pattern skip cohérent pour `TENDERMINT_RPC_URL`, `TENDERMINT_WS_URL`, `TENDERMINT_GRPC_URL`
...
- [x] 7.6.9 Étendre les intégrations live WebSocket pour couvrir exhaustivement les événements, subscriptions et appels publics exposés par `ITendermintWebSocketClient`
- [x] 7.6.10 Étendre les intégrations live gRPC pour couvrir toutes les méthodes publiques exposées par `ITendermintGrpcClient` et valider la forme complète des réponses gRPC
...
- [x] 7.8.2 Flux REST complet : `AddTendermintRest` → `GetHealthAsync` → `GetStatusAsync` → `GetBlockAsync` → `GetValidatorsAsync` — vérifier désérialisation bout en bout
- [x] 7.8.3 Flux WebSocket complet : connexion → souscription `NewBlock` → réception ≥ 1 événement typé `Block` → déconnexion propre
- [x] 7.8.4 Flux gRPC complet : `AddTendermintGrpc` → `PingAsync` → `BroadcastTxAsync` (tx vide, erreur attendue) — vérifier gestion d'exception
- [x] 7.8.5 Skip automatique si `TENDERMINT_RPC_URL` / `TENDERMINT_WS_URL` / `TENDERMINT_GRPC_URL` absents
- [x] 7.8.6 CI E2E gate : step séparé dans `ci.yml` exécuté avec les env vars testnet
- [x] 7.8.7 Étendre les scénarios E2E REST pour refléter la couverture complète des endpoints publics réellement exposés par `ITendermintRestClient`
...
- [x] 7.8.8 Étendre les scénarios E2E WebSocket pour refléter la couverture complète des événements, subscriptions et appels publics exposés par `ITendermintWebSocketClient`
...
- [x] 8.2.10 Étendre la démo REST pour rendre accessibles toutes les méthodes de `ITendermintRestClient`, y compris les capacités `Unsafe` derrière un mode ou un avertissement explicite
...
- [x] 8.3.12 Étendre la démo WebSocket pour refléter exhaustivement tous les événements, subscriptions et appels publics exposés par `ITendermintWebSocketClient`
...
- [x] 8.4.3 Config : `TENDERMINT_GRPC_URL` env var ou `--grpc-url` CLI arg
- [x] 8.4.3b Vérifier que la résolution respecte la priorité CLI arg > env var > défaut testnet :
  ...
  var grpcUrl = args.GetOption("--grpc-url")
      ?? Environment.GetEnvironmentVariable("TENDERMINT_GRPC_URL")
      ?? "cosmoshub.grpc.lava.build";
  ```
- [x] 8.4.4 Enregistrer `AddTendermintGrpc` via DI
